...
def ddp_main(rank, world_size, args):
    ...
    model_name = "google/t5-v1_1-large"  # google/mt5-small"  # t5-v1_1-large  #xl = 2B, xxl = 11B
    ...
    clean_model_name = str.replace(model_name, "/", "=")
    fn = "core_model-" + clean_model_name + ".txt"
    with open(fn, "w") as external_file:
        header_text = f"model = {model_name}, not yet sharded with {param_size_wrapping} parameters\n"
        print(header_text, file=external_file)
        total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)

        print(f"\n--> {model_name} has {total_params} params\n", file=external_file)
        print(f"\nModel arch: \n{model}", file=external_file)
